...
class QuotationApp:
    ...
    def create_widgets(self):
        ...
        btn_clear.pack(side=tk.LEFT, padx=(30, 0))  # 向右移动2个汉字的位置
        ...
        # 历史记录表格
        self.history_tree = ttk.Treeview(history_frame, columns=("时间", "总金额", "操作"), show="headings")
    ...
    def create_quotation_table(self):
        ...
        # 设置列宽
        col_widths = [120, 150, 250, 80, 100, 100, 80]  # 增加操作列宽度
        for col, width in zip(columns, col_widths):
    ...
    def __init__(self, root):
        ...
        self.root.geometry("1200x1000")  # 增加窗口高度
        ...
        self.history_file = "quotation_history.json"  # 历史记录存储文件
        
        # 创建界面
        self.create_widgets()
        
        # 加载历史记录（必须在create_widgets之后）
        self.load_history()
                ...
                with open(self.history_file, 'r', encoding='utf-8') as f:
                    ...
                    for record in history_data:
                        item_id = self.history_tree.insert("", "end", 
                            values=(record['time'], f"{record['total']:,.2f}"))
                        self.history_tree.item(item_id, tags=(str(record),))
        except Exception as e:
            messagebox.showwarning("警告", f"加载历史记录失败：{str(e)}")
    ...
    def save_quotation(self):
        ...
        quotation_data = {
            'time': current_time,
            'total': total,
            'items': [dict(item) for item in self.quotation_items]  # 确保items是可序列化的字典
        }
        
        # 添加到历史记录
        item_id = self.history_tree.insert("", "end", values=(current_time, f"{total:,.2f}"))
        self.history_tree.item(item_id, values=(current_time, f"{total:,.2f}"), tags=(str(quotation_data),))  # 将数据转换为字符串存储
    ...
    def delete_history_item(self, event):
        ...
        if column != "#3":  # 只允许点击操作列
            return
            
        item = self.history_tree.identify_row(event.y)
        
        confirm = messagebox.askyesno("确认", "确定要删除此历史记录吗？")
        if not confirm:
            return
    ...
    def delete_history(self):
        ...
        confirm = messagebox.askyesno("确认", "确定要删除选中的历史记录吗？")
        if not confirm:
            return
    ...
    def clear_quotation(self):
        ...
        messagebox.showinfo("成功", "已清空所有配置！")
    ...
    def load_history_quotation(self, event):
        ...
        # 显示确认对话框
        confirm = messagebox.askyesno("确认", "确定要加载此历史报价单吗？当前报价单将被覆盖！")
        if not confirm:
            return
            
        # 获取保存的报价单数据
        tags = self.history_tree.item(selected_item, "tags")
        if tags and len(tags) > 0:
            try:
                # 将字符串转换回字典
                quotation_data = eval(tags[0])
                if not isinstance(quotation_data, dict):
                    raise ValueError("Invalid data format")
            except Exception as e:
                messagebox.showwarning("警告", f"历史报价单数据解析失败：{str(e)}")
                return
        else:
            messagebox.showwarning("警告", "历史报价单数据已损坏！")
            return
